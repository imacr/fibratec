...
useEffect(() => {
  fetch(`${API_URL}/tipos_mantenimiento`)
    .then(res => res.json())
    .then(data => setTiposMantenimiento(data))
  ...
  const handleRegistrar = (p) => {
    Swal.fire({
      ...
      text: `¿Deseas registrar el mantenimiento menor para la unidad ${p.cve} - ${p.marca} ${p.clase_tipo}?`,
      icon: "question",
      ...
      return (
        <div key={p.id_mantenimiento_programado} className={`unidad-card ${alerta}`}>
          ...
          <div className="file-buttons">
            {p.url_documento && <button className="btn btn-outline-danger btn-sm" onClick={() => setFileModalUrl(`${API_URL}/${p.url_documento}`)}>Ver Documento</button>}
          </div>
