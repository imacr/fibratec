...
export default function SolicitudFallaPaso1y2() {
  ...
  const cargarSolicitudes = async () => {
    try {
      ...
      // Solo solicitudes pendientes o aprobadas no completadas
      ...
      // Si hay pendiente, pero aún no aprobada, mostrar mensaje
  ...
  useEffect(() => {
    ...
    const cargarDatos = async () => {
      try {
        const [unidadesRes, piezasRes, marcasRes, lugaresRes] = await Promise.all([
          fetch(`${API_URL}/unidades`).then(r => r.json()),
  ...
  const handleFileChange = e => setFallaData(prev => ({ ...prev, url_comprobante: e.target.files[0] }));
...
const handleSubmitFalla = async id_solicitud => {
  ...
  // Agregar el PDF correctamente
  if (fallaData.url_comprobante) {
    formDataFalla.append("comprobante", fallaData.url_comprobante); // <--- clave exacta
  }
